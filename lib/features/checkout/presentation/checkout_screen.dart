...
class _CheckoutScreenState extends ConsumerState<CheckoutScreen> {
  ...
  Future<void> _refreshQuote() async {
    ...
    quote = await ref.read(commerceRepositoryProvider).quote(
  ...
  Future<void> _submit() async {
    if (busy || cart == null || address == null || payment == null || currency == null) {
      return;
    }
    setState(() => busy = true);
    try {
  ...
  Widget build(BuildContext context) {
    if (loading) {
      return const Scaffold(body: SpikeLoading());
    }
    if (cart == null) {
      return Scaffold(body: SpikeErrorState(onRetry: _load));
    }
    ...
    final deliveryInCartCurrency = (quote?.shippingUsd ?? 0) * cart!.exchangeRateFromUsd;
    final grand = cart!.subtotal + deliveryInCartCurrency;
                  ...
                  Container(
                    ...
                    child: Row(
                      children: [
                        ...
                        const Expanded(
                          child: Text(
                            'رسوم التوصيل محسوبة من الخادم حسب التغطية والمسافة',
                            style: TextStyle(fontSize: 10, fontWeight: FontWeight.w600),
                          ),
                        ),
                        if (quote != null)
                          Text(
                            _money(deliveryInCartCurrency, cart!.currencyCode),
                          ...
                          InkWell(
                            ...
                            child: Container(
                              ...
                              decoration: BoxDecoration(
                                ...
                                borderRadius: BorderRadius.circular(17),
                                border: Border.all(
                                  color: payment?.method == method.method
                    ...
                    Container(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          const Text(
                            'بعد تأكيد الطلب سترفع سند الحوالة من قسم طلباتي.',
                            style: TextStyle(fontSize: 9, color: spikeMuted, height: 1.5),
                          ),
                        ],
                      ),
                    ),
                    ...
                    child: Column(
                      children: [
                        _InvoiceLine(
                          ...
                          value: _money(cart!.subtotal, cart!.currencyCode),
                        ),
                        _InvoiceLine(
                          label: 'رسوم مكتب التوصيل',
                          value: quote == null ? 'تحسب عند التأكيد' : _money(deliveryInCartCurrency, cart!.currencyCode),
                        ),
                        Container(
                          constraints: const BoxConstraints(minHeight: 48),
