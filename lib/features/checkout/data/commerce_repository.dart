...
class AddressModel {
  ...
  final String id, cityId, cityName, label, recipientName, phone, addressLine, googleMapsUrl;
  final bool isActive;
...
class PaymentMethodModel {
  const PaymentMethodModel({required this.method, required this.instructions});
  final String method, instructions;
...
class PaymentAccountModel {
  ...
  final String id, name, accountName, accountNumber, instructions;

  factory PaymentAccountModel.fromJson(Map<String, dynamic> j) => PaymentAccountModel(
        id: '${j['id'] ?? ''}',
        name: '${j['name'] ?? ''}',
        accountName: '${j['account_name'] ?? ''}',
        accountNumber: '${j['account_number'] ?? ''}',
...
class CheckoutBannerModel {
  const CheckoutBannerModel({
    required this.id,
  ...
  final String id, imageUrl, targetType, targetId, targetUrl;

  factory CheckoutBannerModel.fromJson(Map<String, dynamic> j) => CheckoutBannerModel(
        id: '${j['id'] ?? ''}',
        imageUrl: ApiConfig.resolveMedia('${j['image_url'] ?? ''}'),
...
class CurrencyModel {
  ...
  factory CurrencyModel.fromJson(Map<String, dynamic> j) => CurrencyModel(
...
class DeliveryQuote {
  const DeliveryQuote({required this.shippingUsd, required this.shippingYerOld, required this.quotes});
  final double shippingUsd, shippingYerOld;
  final List<Map<String, dynamic>> quotes;

  factory DeliveryQuote.fromJson(Map<String, dynamic> j) => DeliveryQuote(
        shippingUsd: double.tryParse('${j['shipping_usd'] ?? 0}') ?? 0,
        shippingYerOld: double.tryParse('${j['shipping_yer_old'] ?? 0}') ?? 0,
...
class CommerceRepository {
  ...
  Future<List<CityModel>> cities() async {
    final d = await _api.get('/cities');
    return (d['cities'] as List? ?? const [])
        .whereType<Map>()
  ...
  Future<List<AddressModel>> addresses() async {
    final d = await _api.get('/addresses', auth: true);
    return (d['addresses'] as List? ?? const [])
        .whereType<Map>()
  ...
  Future<AddressModel> createAddress({
    ...
    final d = await _api.post('/addresses', auth: true, data: {
      'city_id': cityId,
      'label': label,
      'recipient_name': recipientName,
      'phone': phone,
      'address_line': addressLine,
      'google_maps_url': googleMapsUrl.trim().isEmpty ? null : googleMapsUrl.trim(),
      'is_active': isActive,
    });
    return AddressModel.fromJson(Map<String, dynamic>.from(d['address'] as Map));
  }
  ...
  Future<AddressModel> updateAddress(
    ...
    final d = await _api.put('/addresses/$id', auth: true, data: {
      'city_id': cityId,
      'label': label,
      'recipient_name': recipientName,
      'phone': phone,
  ...
  Future<void> activateAddress(String id) async {
    await _api.post('/addresses/$id/activate', auth: true);
  }

  Future<void> deleteAddress(String id) async {
    await _api.delete('/addresses/$id', auth: true);
  }

  Future<List<PaymentMethodModel>> paymentMethods() async {
    final d = await _api.get('/payment-methods');
  ...
  Future<List<PaymentAccountModel>> paymentAccounts() async {
    final d = await _api.get('/payment-accounts');
    return (d['accounts'] as List? ?? const [])
        .whereType<Map>()
        .map((e) => PaymentAccountModel.fromJson(Map<String, dynamic>.from(e)))
        .toList();
  }

  Future<List<CheckoutBannerModel>> checkoutBanners() async {
    final d = await _api.get('/banners', query: {'placement': 'checkout'});
    return (d['banners'] as List? ?? const [])
        .whereType<Map>()
        .map((e) => CheckoutBannerModel.fromJson(Map<String, dynamic>.from(e)))
        .where((e) => e.imageUrl.trim().isNotEmpty)
        .toList();
  }

  Future<List<CurrencyModel>> currencies() async {
    final d = await _api.get('/currencies');
    return (d['currencies'] as List? ?? const [])
        .whereType<Map>()
        .map((e) => CurrencyModel.fromJson(Map<String, dynamic>.from(e)))
        .toList();
  }

  Future<DeliveryQuote> quote({required String addressId, required CartSnapshot cart}) async =>
      DeliveryQuote.fromJson(await _api.post('/checkout/delivery-quote-v2', auth: true, data: {
  ...
  Future<List<Map<String, dynamic>>> orderTimeline(String id) async {
    final d = await _api.get('/customer-orders/$id/timeline', auth: true);
    return (d['timeline'] as List? ?? const [])
        .whereType<Map>()
        .map((e) => Map<String, dynamic>.from(e))
        .toList();
  }

  Future<void> uploadReceipt(String orderId, String filePath) async {
    final up = await _api.uploadFile('/uploads/receipt', filePath: filePath);
    final ref = '${up['key'] ?? up['path'] ?? up['url'] ?? ''}';
    if (ref.isEmpty) throw const ApiException('تعذر رفع السند');
    await _api.post('/customer-media/orders/$orderId/receipt', auth: true, data: {'receipt_url': ref});
  }

  Future<void> requestReturn({required String orderItemId, required int quantity, required String reason}) async {
    await _api.post('/customer-orders/returns', auth: true, data: {
  ...
  Future<List<Map<String, dynamic>>> returnsHistory() async {
    final d = await _api.get('/customer-orders/returns/list', auth: true);
    return (d['returns'] as List? ?? const [])
        .whereType<Map>()
        .map((e) => Map<String, dynamic>.from(e))
        .toList();
  }

  Future<List<Map<String, dynamic>>> refundsHistory() async {
    final d = await _api.get('/refunds', auth: true);
    return (d['refunds'] as List? ?? const [])
        .whereType<Map>()
        .map((e) => Map<String, dynamic>.from(e))
        .toList();
  }

  Future<List<Map<String, dynamic>>> cartBanners() async {
    final d = await _api.get('/banners', query: {'placement': 'cart'});
    return (d['banners'] as List? ?? const [])
        .whereType<Map>()
        .map((e) => Map<String, dynamic>.from(e))
        .toList();
