...
class _OffersScreenState extends ConsumerState<OffersScreen> {
  ...
  int _discount(ProductModel p) {
    ...
    final current = p.price;
    if (original == null || original <= current || current <= 0) return 0;
    return ((original - current) / original * 100).round();
  }

  List<ProductModel> _apply(List<ProductModel> all) {
    var items = all.where((p) => _discount(p) > 0).toList();
    if (minDiscount > 0) {
      items = items.where((p) => _discount(p) >= minDiscount).toList();
    }
    if (price == 'under5') {
      items = items.where((p) => p.price < 5).toList();
    }
    if (price == '5to25') {
      items = items.where((p) => p.price >= 5 && p.price <= 25).toList();
    }
    if (price == 'over25') {
      items = items.where((p) => p.price > 25).toList();
    }
    if (minRating > 0) {
    ...
    if (sort == 'newest') {
      ...
      items.sort((a, b) => _discount(b).compareTo(_discount(a)));
    }
    return items;
  }
            ...
            child: Column(
              ...
              children: [
                ...
                _FilterGroup(
                  title: 'نسبة الخصم',
                  children: [
                    for (final value in const [0, 10, 20, 30, 40])
                      _Chip(
                        value == 0 ? 'كل العروض' : 'خصم $value% فأكثر',
                        localDiscount == value,
                        () => setLocal(() => localDiscount = value),
                      ),
                  ],
                ),
                _FilterGroup(
  ...
  Widget build(BuildContext context) {
    final state = ref.watch(offersProductsProvider);
    ...
    return SafeArea(
      child: state.when(
        ...
        error: (error, _) => SpikeErrorState(
          ...
          onRetry: () => ref.invalidate(offersProductsProvider),
        ),
        data: (all) {
                        ...
                        child: Padding(
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              const SizedBox(height: 5),
                              const Text(
                                'سنظهر الخصومات هنا فور إضافتها.',
                                textAlign: TextAlign.center,
                    ...
                    : GridView.builder(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          return SpikeProductCard(
                            ...
                            onAdd: product.cheapestVariant == null
                                ? null
                                : () async {
                                    try {
