...
class _SupportChatScreenState extends ConsumerState<SupportChatScreen> {
  ...
  Future<void> _openService(String scheme, String value) async {
    final cleaned = scheme == 'tel'
        ? value.replaceAll(RegExp(r'\s+'), '')
        : value.replaceAll(RegExp(r'[^0-9]'), '');
    final uri = Uri.parse(
      scheme == 'tel' ? 'tel:$cleaned' : 'https://wa.me/$cleaned',
    );
      ...
      body: SafeArea(
        child: Column(
          children: [
                ...
                child: Row(
                  children: [
                    Container(
                      ...
                      height: 42,
                      decoration: BoxDecoration(
                        color: spikeRed.withValues(alpha: .10),
                ...
                children: [
                  Expanded(
                    child: _ServiceButton(
                      ...
                      label: 'اتصال',
                      value: '${service['customer_service_phone'] ?? ''}',
                      onTap: () => _openService(
                        'tel',
                        '${service['customer_service_phone'] ?? ''}',
                      ),
                    ),
                  ),
                  ...
                  Expanded(
                    child: _ServiceButton(
                      ...
                      label: 'واتساب',
                      value: '${service['customer_service_whatsapp'] ?? ''}',
                      onTap: () => _openService(
                        'wa',
                        '${service['customer_service_whatsapp'] ?? ''}',
                      ),
                    ),
                  ),
                ],
            ...
            Expanded(
              child: session.when(
                loading: () => loading ? const SpikeLoading() : _loginRequiredChat(),
                error: (_, __) => _loginRequiredChat(),
            ...
            session.maybeWhen(
              data: (loggedIn) => loggedIn ? _composeBar(dark, scheme) : const SizedBox.shrink(),
  ...
  Widget _loginRequiredChat() => Center(
        child: Padding(
          padding: const EdgeInsets.symmetric(horizontal: 34),
          child: Column(
            mainAxisSize: MainAxisSize.min,
            children: [
              const Icon(LucideIcons.messageCircle, size: 40),
              ...
              const Text(
                'الاتصال والواتساب متاحان بدون تسجيل دخول.',
                textAlign: TextAlign.center,
  ...
  Widget _chatArea(bool dark) {
    if (loading) return const SpikeLoading();
    if (messages.isEmpty) {
      return const Center(
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              SizedBox(height: 8),
              Text(
                'اكتب رسالتك للإدارة وسيظهر الرد هنا في نفس المحادثة.',
                textAlign: TextAlign.center,
                style: TextStyle(fontSize: 11, height: 1.6, color: spikeMuted),
              ),
            ],
          ),
        ),
          ...
          return Align(
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                color: mine
                    ? spikeRed
                    : (dark ? spikeDarkPanel : const Color(0xFFE9E9E9)),
  ...
  Widget _composeBar(bool dark, ColorScheme scheme) => Container(
        padding: const EdgeInsets.fromLTRB(
          SpikeSpacing.page,
          8,
          SpikeSpacing.page,
        ...
        decoration: BoxDecoration(
          ...
          border: Border(
            top: BorderSide(color: scheme.onSurface.withValues(alpha: .08)),
          ),
        ),
        child: SafeArea(
          top: false,
            ...
            children: [
              Expanded(
                child: TextField(
                  ...
                  decoration: InputDecoration(
                    hintText: 'اكتب رسالة...',
                    filled: true,
                    fillColor: dark ? spikeDarkPanel : const Color(0xFFE7E7E7),
              ...
              SizedBox(
                ...
                child: IconButton.filled(
                  style: IconButton.styleFrom(backgroundColor: spikeRed),
...
class _ServiceButton extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return OutlinedButton.icon(
      ...
      label: Text(
        enabled ? label : '$label غير متاح',
        style: const TextStyle(fontSize: 10),
      ),
      style: OutlinedButton.styleFrom(minimumSize: const Size(0, 39)),
    );
  }
}
