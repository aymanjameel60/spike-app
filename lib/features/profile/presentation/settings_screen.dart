...
class _SettingsScreenState extends ConsumerState<SettingsScreen> {
                ...
                children: [
                  _SettingsSection(
                    ...
                    child: Row(
                      children: [
                        Expanded(
                          child: _ThemeOption(
                            title: ar ? 'الوضع الفاتح' : 'Light mode',
                  ...
                  _SettingsSection(
                    ...
                    subtitle: ar
                        ...
                        : 'Choose the currency used to display prices',
                    child: currencies.when(
                      ...
                      data: (items) {
                        final selected = items
                            .where((currency) => currency.code == settings.currency)
                            .firstOrNull;
                        return Column(
                          ...
                          children: [
                            ...
                            Text(
                              ar
                                  ? 'سيتم ربط أسعار الصرف من لوحة التحكم عند توصيل الواجهة بالباك إند.'
                                  : 'Exchange rates will be supplied by the backend when the UI is connected.',
                              style: const TextStyle(
                                fontSize: 9,
                                color: spikeMuted,
                                height: 1.5,
                  ...
                  Container(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        Text(
                          ar
                              ? 'سيتم حذف الحساب والبيانات المرتبطة به نهائياً بعد التأكيد.'
                              : 'Your account and associated data will be permanently deleted after confirmation.',
                          style: const TextStyle(
                            fontSize: 9.5,
                            color: spikeMuted,
                            height: 1.55,
                        ...
                        OutlinedButton(
                          style: OutlinedButton.styleFrom(
                            ...
                            side: const BorderSide(color: spikeRed),
                          ...
                          onPressed: () {
                            if (!session) {
                              showSpikeToast(
                                context,
                                ar
                                    ? 'سجّل الدخول أولاً لحذف الحساب'
                                    : 'Sign in first to delete your account',
                              );
                              context.push(
                                '/login?next=${Uri.encodeComponent('/settings')}',
                              );
                              return;
                            }
                            _showDeleteModal(ar);
  ...
  Future<void> _showDeleteModal(bool ar) async {
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                Text(
                  ar
                      ? 'هذا الإجراء نهائي. سيتم تسجيل خروجك وحذف بيانات الحساب.'
                      : 'This action is permanent. You will be signed out and the account will be deleted.',
                  textAlign: TextAlign.center,
                  style: const TextStyle(
                    fontSize: 11,
                    color: spikeMuted,
                    ...
                    onPressed: _deleting
                        ...
                        : () async {
                            ...
                            try {
                              ...
                              showSpikeToast(context, 'تم حذف الحساب');
                              context.go('/');
                            } catch (error) {
                              if (mounted) {
                                showSpikeToast(context, error.toString());
                              }
                            } finally {
                              if (mounted) setState(() => _deleting = false);
                              setLocal(() => _deleting = false);
                            }
                          },
                    child: Text(
                      _deleting
                          ? (ar ? 'جاري الحذف...' : 'Deleting...')
                          : (ar ? 'نعم، حذف الحساب' : 'Yes, delete account'),
                    ),
                  ),
                ),
                const SizedBox(height: 8),
            ...
            children: [
              Row(
                children: [
                  Expanded(
                    child: Text(
                      ar ? 'اختر العملة' : 'Choose currency',
                      style: const TextStyle(
                        fontSize: 18,
                        fontWeight: FontWeight.w700,
                      ),
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    subtitle,
                    style: const TextStyle(fontSize: 8, color: spikeMuted),
                  ),
                ],
              ),
            ),
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        shape: BoxShape.circle,
        border: Border.all(
          color: selected ? color : spikeMuted,
          width: 1.5,
        ),
      ),
      child: selected
